...
/** Super Admin login seeds all slots; backfill from admin when switching portals. */
function backfillSuperAdminSession(slot: PortalSlot): PortalSession | null {
  if (slot === "admin") return null;
  const admin = getSession("admin");
  if (!admin?.token || admin.user?.role !== "super_admin") return null;
  setSession(slot, admin, { notify: false });
  return admin;
}

export function resolveSession(slot: PortalSlot): PortalSession | null {
  return getSession(slot) ?? backfillSuperAdminSession(slot);
}
